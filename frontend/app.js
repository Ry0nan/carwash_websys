...
async function loadSession() {
  ...
  try {
    ...
    $('sessionBadge').textContent = `Logged in: ${state.authUser.name || state.authUser.email}`;
...
async function hydrateDashboard() {
  ...
  const [customers, vehicles, orders, report] = await Promise.allSettled([
    api('/customers'), api('/vehicles'), api('/job-orders'), api(`/reports/daily?date=${date}`),
  ]);
  const kpis = [
  ...
  $('dashboardCards').innerHTML = kpis.map(([name, value]) => `<article class="kpi"><h3>${name}</h3><strong>${value}</strong></article>`).join('');
}
...
async function loadCustomers() {
  ...
  toRows('customersTable', ['id', 'full_name', 'contact_number', 'created_at'], payload.data || []);
...
async function loadVehicles() {
  ...
  toRows('vehiclesTable', ['id', 'customer_id', 'plate_number', 'vehicle_category', 'vehicle_size'], payload.data || []);
...
async function loadServices() {
  ...
  toRows('servicesTable', ['id', 'service_name', 'vehicle_category', 'service_group', 'is_active'], state.services, (s) => {
    const { ok, reason } = selectedServiceRuleStatus(s, selectedGroups, vCategory);
    return `<button class="btn btn-ghost" ${ok ? '' : 'disabled'} onclick="selectService(${s.id})">${ok ? 'Select' : reason}</button>`;
  });

  toRows('checkinServicesTable', ['id', 'service_name', 'vehicle_category', 'service_group'], state.services, (s) => {
    const { ok, reason } = selectedServiceRuleStatus(s, selectedGroups, vCategory);
    return `<button class="btn btn-ghost" ${ok ? '' : 'disabled'} onclick="addCheckinService(${s.id})">${ok ? 'Add' : reason}</button>`;
  });
}
...
window.addCheckinService = (id) => {
  ...
  if (!svc) return;
  const selectedGroups = new Set(state.checkinItems.filter((i) => i.service_group).map((i) => i.service_group));
  ...
  state.checkinItems.push({ service_id: svc.id, item_name: svc.service_name, service_group: svc.service_group });
  renderSelectedItems();
  loadServices();
};
...
async function loadOrders() {
  ...
  const payload = res.data;
  toRows('ordersTable', ['id', 'customer_id', 'vehicle_id', 'payment_mode', 'status', 'created_at'], payload.data || [],
    (row) => `<button class="btn btn-ghost" onclick="viewOrder(${row.id})">View</button>`);
}
window.viewOrder = async (id) => {
  const res = await api(`/job-orders/${id}`);
  $('orderDetailOutput').textContent = JSON.stringify(res.data, null, 2);
};
...
async function loadReport() {
  ...
  $('reportSummary').innerHTML = Object.entries(summary).map(([k, v]) => `<article class="kpi"><h3>${k}</h3><strong>${typeof v === 'object' ? JSON.stringify(v) : v}</strong></article>`).join('');
  toRows('reportTable', ['job_order_id', 'customer_name', 'plate_number', 'payment_mode', 'total_amount', 'status'], res.data.orders || []);
}
...
function bindForms() {
  ...
  $('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await api('/auth/login', { method: 'POST', body: JSON.stringify(Object.fromEntries(fd.entries())) });
  ...
  $('customerForm').addEventListener('submit', async (e) => {
    ...
    try {
      const fd = new FormData(e.target);
      await api('/customers', { method: 'POST', body: JSON.stringify(Object.fromEntries(fd.entries())) });
      e.target.reset();
      await loadCustomers();
  ...
  $('vehicleForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (payload.vehicle_category === 'MOTOR') payload.vehicle_size = null;
  ...
  $('customItemForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const payload = Object.fromEntries(new FormData(e.target).entries());
    if (payload.price_status === 'FIXED') payload.unit_price = Number(payload.unit_price || 0);
  ...
  $('createJobBtn').addEventListener('click', async () => {
    try {
      const fd = new FormData($('jobOrderForm'));
      const payload = Object.fromEntries(fd.entries());
      payload.customer_id = Number(payload.customer_id);
      payload.vehicle_id = Number(payload.vehicle_id);
      payload.leave_vehicle = $('jobOrderForm').leave_vehicle.checked;
      payload.waiver_accepted = $('jobOrderForm').waiver_accepted.checked;
      if (payload.leave_vehicle && payload.waiver_accepted) payload.waiver_accepted_at = new Date().toISOString().slice(0, 19).replace('T', ' ');
      payload.items = state.checkinItems.map((item) => item.service_id ? { service_id: item.service_id } : item);

      const res = await api('/job-orders', { method: 'POST', body: JSON.stringify(payload) });
